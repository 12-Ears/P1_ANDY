...
rename_freq<-function(column_names) {
  freq<-sapply(column_names, function(x) str_extract(x,"f\\..*(?=\\.\\b)")) %>%
    gsub("\\.","",.)
  c(column_names[1:14],freq[15:length(column_names)])
...
### Renaming frequency columns

```{r}

new_name<-rename_freq(names(a_data))
colnames(a_data)<-new_name
